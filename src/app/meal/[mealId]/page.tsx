...
export default async function Mealpage({params}:{params:{mealId:string}}) {
    const mealDetailes:MealDetailes= await getmealDetailes(params.mealId)
  ...
  <div className="w-[400px] h-[400px] bg-gray-200 flex items-center justify-center rounded-lg shadow-lg md:mx-10 mt-10">
    No Image Available
     ...
     <h1>{mealDetailes.strMeal}</h1>
     <p>{mealDetailes.strInstructions}</p>
     <Button variant='default' className='mt-4 mr-3'>
        <a href={mealDetailes.strYoutube} target='_blank' rel='noopener noreferrer'> YouTube</a>
     </Button>
     <Button variant='destructive'>
        <a href={mealDetailes.strSource} target='_blank' rel='noopener noreferrer'> Source</a>
     </Button>
     </div>
